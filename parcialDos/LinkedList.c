...
/** \brief Inicializa el nodo Iterador en el primer nodo.
 *
 * \param Valida que la lista sea distinta de NULL.
 * \param le pasamos a al campo iteratorNode el primer nodo.
 * \return 0 si esta bien.
 *
 */

static int startIterator(LinkedList* this)
{
    int auxReturn = -1;
    if(this!=NULL)
    {
        this->iteratorNode = this->pFirstNode;
        auxReturn=0;
    }
    return auxReturn;
}
 ...
 *
 * \param Valida que la lista sea distinta de NULL.
 * \param Declaramos un puntero de void y le pasamos el elemento de el nodo iterador.
 * \param al nodo iterador le pasamos el siguiente nodo.
 * \return El elemento.
 *
 */
...
static void* getNext(LinkedList* this)
{
    ...
    if(this!=NULL)
    {
        auxReturn=this->iteratorNode->pElement;
        this->iteratorNode = this->iteratorNode->pNextNode;
    }
    return auxReturn;
}
...
/** \brief Contamos los elementos de la linkedList segun un criterio.
 * \param le pasamos un puntero de LinkedList y un puntero a funcion.
 * \param con un if validamos, recorremos con un for y el elemento se lo pasamos a la funcion criterio y va acumulando.
 * \return retorna total.
 */

int ll_count(LinkedList* this, int(*fn)(void* element, char*),char* criterio)
{
    int i;
    void* auxElement;
    int cantidad;
    ...
    if(this!=NULL && fn!=NULL)
    {
        ...
        for(i=0;i<ll_len(this);i++)
        {
            ...
            if(auxElement!=NULL)
            {
                cantidad = fn(auxElement,criterio);
